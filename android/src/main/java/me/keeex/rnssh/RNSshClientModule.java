...
//          map.putString("value", String.valueOf(charVal));
    ...
    /**
     * Returns true if the given Uri is related to android context:// schema
     * @param uri the file Uri
     * @return true if the given Uri is related to android context:// schema
     */
    private boolean isUriContext(Uri uri) {
        return Objects.equals(uri.getScheme(), "content");
    }

    /**
     * Get file name from URI
     * See <a href="https://stackoverflow.com/questions/5568874/how-to-extract-the-file-name-from-uri-returned-from-intent-action-get-content">this thread on StackOverlflow</a>
     * @param uri The file Uri
     ...
     * @param uri The file Uri
     * @return the file size, or SftpProgressMonitor.UNKNOWN_SIZE if the file size is unknown
     */
    private long getFileSize(Uri uri) {
        AssetFileDescriptor fileDescriptor = null;
        try {
            fileDescriptor = reactContext.getApplicationContext().getContentResolver().openAssetFileDescriptor(uri, "r");
            long fileLength = SftpProgressMonitor.UNKNOWN_SIZE;
            ...
            return SftpProgressMonitor.UNKNOWN_SIZE;
        }
    }

    @ReactMethod
    public void sftpUpload(final String localFilePath, final String remoteFilePath, final String key, final Callback callback) {
        new Thread(new Runnable()  {
            public void run() {
                try {
                    SSHClient client = clientPool.get(key);
                    ...
                    Uri localFileUri = Uri.parse(localFilePath);
                    boolean isLocalFilePathFromContext = isUriContext(localFileUri);
                    String fileName = getFileName(localFileUri);

                    if(isLocalFilePathFromContext) {
                        InputStream inputStream = reactContext.getApplicationContext().getContentResolver().openInputStream(localFileUri);
                        channelSftp.put(inputStream, remoteFilePath + '/' + fileName, new progressMonitor(key, "UploadProgress", getFileSize(localFileUri)), ChannelSftp.OVERWRITE);
                    }
                    else {
                        channelSftp.put(localFilePath, remoteFilePath + '/' + (new File(localFilePath)).getName(), new progressMonitor(key, "UploadProgress"), ChannelSftp.OVERWRITE);
                    }
                    callback.invoke();
                } catch (SftpException error) {
    ...
    private class progressMonitor implements SftpProgressMonitor {
        private long max = SftpProgressMonitor.UNKNOWN_SIZE;
        ...
        public progressMonitor(String key, String name, long fileSize) {
            this.key = key;
            this.name = name;
            this.max = fileSize;
        }
        ...
        public void init(int operation, String src, String dest, long fileSize) {
            this.max = fileSize == SftpProgressMonitor.UNKNOWN_SIZE ? this.max : fileSize;
        }
